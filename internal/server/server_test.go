...
import (
	"context"
	"fmt"
	"godistrserv/internal/auth"
	"godistrserv/internal/config"
	"godistrserv/internal/log"
	"net"
	"os"
	"testing"
...
func TestServer(t *testing.T) {
	for scenario, fn := range map[string]func(
		...
		//teardown просто закрывает всё (соединения)
		t.Run(scenario, func(t *testing.T) {
			rootClient, nobodyClient, cfg, teardown := setupTest(t, nil)
			defer teardown()
			//defer отрабатывает по завершению работы анонимной функции
...
func setupTest(t *testing.T, fn func(*Config)) (
	...
	newClient := func(crtPath, keyPath string) (
		*grpc.ClientConn,
		...
		require.NoError(t, err)
		/*
			создание нового grpc TLS клиента
		*/
